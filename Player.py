...
class Revised():
    ...
    def __post_init__(self):
        ...
        self.fell_at_time = 0
        self.count = 0
        self.jump_values_y = [(gravity*1.3)-x for x in range(0, gravity*3, 2)]
        self.jump_values_x = [2 for x in self.jump_values_y]

    def detect_vertical_movement(self):
        if not(self.scrolling_y):
            if self.last_y > self.hitbox.y:
                self.moving_up = True
                self.moving_down = False
            elif self.last_y < self.hitbox.y:
                self.moving_up = False
    ...
    def update_counter(self):
        if pygame.time.get_ticks() - self.last_updated >= 75:
            if not(self.jumping) and not(self.falling) and not(self.isIdle):
    ...
    def move(self, level, shift_y, right=False, left=False, jump=False):
        if not(self.scrolling_y):
        ...
        self.isIdle = not(left or right) and not(self.jumping or self.falling)
    ...
    def draw_hitbox(self):
        pygame.draw.rect(screen, rect=self.hitbox, color=(0, 0, 0), width=1)
